...
https://github.com/user-attachments/assets/3e9d6b3f-2369-4978-a03e-1882f6089e30

*Вход → переключение темы → загрузка DOCX и фоновая индексация → поиск по
фрагментам с оценками релевантности → ответ со ссылкой на документ и его
раздел → вопрос, ответа на который в базе нет. Модель `qwen3:14b` на Apple
M4 Max, запись без монтажа и ускорения — латентность в кадре настоящая.
Тот же ролик файлом — [docs/assets/demo.mp4](docs/assets/demo.mp4).*
...
- [Проверить за пять минут](#проверить-за-пять-минут) · [Быстрый старт](#быстрый-старт) · [Веб-интерфейс](#веб-интерфейс) · [Аутентификация](#аутентификация) · [API](#api) · [Конфигурация](#конфигурация)
...
Нужны только Docker и [Ollama](https://ollama.com) на хосте.
...
Третья команда создаёт пользователя `demo@example.com` / `demo-password`,
базу знаний «Внутренние документы» и загружает в неё демо-документы из
[docs/demo/](docs/demo/) — регламент дежурств, политику командировок и
онбординг в трёх разных форматах. Скрипт ждёт окончания индексации и
печатает, чем всё закончилось; повторный запуск ничего не дублирует.
...
Дальше можно загрузить свой PDF или DOCX кнопкой «Загрузить документ» и
задать вопрос по нему — статус индексации обновляется в списке сам.

> Модель `qwen3:4b` взята как самая лёгкая (2.6 ГБ). На машине с 24+ ГБ
> памяти качество заметно выше на `standard`-профиле: `ollama pull qwen3:14b`
> и `HARDWARE_PROFILE_OVERRIDE=standard docker compose up -d`.
