...
DEMO_KB_NAME = "Внутренние документы"
...
def _knowledge_base(client: httpx.Client) -> str:
    existing = client.get("/knowledge-bases")
    existing.raise_for_status()
    for kb in existing.json():
        if kb["name"] == DEMO_KB_NAME:
            print(f'База знаний "{DEMO_KB_NAME}" уже есть')
            return kb["id"]
    ...
    print(f'База знаний "{DEMO_KB_NAME}" создана')
    return created.json()["id"]


def _upload(client: httpx.Client, kb_id: str) -> int:
